...
\begin{proofsec}
    ...
    \end{equation*}
    %
    If \subcref{enum:R0-closure-of-point} holds, then $q(\closure{\{x\}}) = q([x]_\equiv) = \{q(x)\}$, showing that singletons in $X/{\equiv}$ are closed. Conversely, if the quotient is $T_1$ then $q(\closure{\{x\}}) = \closure{\{q(x)\}} = \{q(x)\}$, so $\closure{\{x\}} \subseteq [x]_\equiv$.
\end{proofsec}
\end{proof}
