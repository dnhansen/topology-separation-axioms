...
\begin{proof}
    We first show that all open sets are saturated. Let $U$ be an open set of $X$, and let $x \in U$. If $x \equiv x'$, then $U$ is also a neighbourhood of $x'$, so $x' \in U$. It follows that $q\inv(q(x)) \subseteq U$, and so
    %
    \begin{equation*}
        U = \bigunion_{x \in U} q\inv(q(x)).
    \end{equation*}
    %
    Hence $U$ is a union of fibres of $q$, and thus it is saturated with respect to $q$. The complement $U^c$ is the union of all equivalence classes $q(x)$ for $x \in U^c$, so closed sets are also saturated. Since $q$ is a quotient map, it follows that it takes saturated open (closed) subsets of $X$ to open (closed) subsets of $X/{\equiv}$,\footnote{This is easy to show. See also \textcite[Proposition~3.60]{leetopological}.} and hence it is both open and closed.
...
\begin{proposition}
    ...
    Let $X$ be a topological space, and let $q \colon X \to X/{\equiv}$ be the quotient map onto its $T_0$-identification. For every $T_0$-space $Y$ the map
    %
    \begin{align*}
        \Phi \colon C(X/{\equiv},Y) &\to C(X,Y), \\
        f &\mapsto f \circ q,
    \end{align*}
    %
    is a bijection. If $Y$ is a $T_0$ topological vector space\footnotemark, then $\Phi$ is a linear isomorphism, and it is a ring isomorphism in the case $Y = \reals$.
\end{proposition}
...
\begin{proof}\footnotetext{One can show that a topological group is $T_1$ if it is $T_0$, and furthermore is always regular, so a $T_0$ topological group is $T_3$.}
    ...
    Thus every $g \in C(X,Y)$ descends to a unique map $\tilde{g} \in C(X/{\equiv},Y)$ with $g = \tilde{g} \circ q$. Conversely, every $f \in C(X/{\equiv},Y)$ uniquely determines a map $f \circ q \in C(X,Y)$. Hence $\Phi$ is a bijection.
...
\begin{proof}
\begin{proofsec}
    \item[\subcref{enum:regular-space} $\implies$ \subcref{enum:closed-nhood-inside-nhood}]
